...
#         # await in_stream(message)
#         # await in_stream.done()

#         # return SynthesisResult(
#         #     self.experimental_mp3_streaming_output_generator(
#         #         out_stream, chunk_size, create_speech_span
#         #     ),  # should be wav
#         #     lambda seconds: self.get_message_cutoff_from_voice_speed(
#         #         message, seconds, self.words_per_minute
#         #     ),
...
class UpdatedPlayHtSynthesizer(BaseSynthesizer[UpdatedPlayHtSynthesizerConfig]):
    ...
    async def create_speech(
        ...
        # print('create_speech_for****create_speech_for****create_speech_for****')
        # print(message)
        # print('create_speech_for****create_speech_for****create_speech_for****')

        # stream = self.client.tts(message.text, options)
        # text_message, stream = self.client.stream_tts_input(message, options)
        stream = self.client.stream_tts_input(message, options)
        # print('stream_tts_input****stream_tts_input****')
        # print(stream)
        # print('stream_tts_input****stream_tts_input****')
        text_message = await anext(stream)
        print('text_message****text_message****'*3)
        print(text_message[0])
        print('text_message****text_message****'*3)
        text_message = BaseMessage(text=text_message[0])


        return text_message.text, SynthesisResult(
            self.experimental_mp3_streaming_output_generator(
                stream, chunk_size, create_speech_span
            ),  # should be wav
            lambda seconds: self.get_message_cutoff_from_voice_speed(
                text_message, seconds, self.words_per_minute
                )
            # lambda seconds: text_message,
            # lambda x: 100,
