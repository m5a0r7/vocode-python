...
class DeepgramTranscriber(BaseAsyncTranscriber[DeepgramTranscriberConfig]):
    ...
    async def is_speech_final(
        ...
        if self.transcriber_config.interrupt_on_blockers:
            ...
            try:
                is_interrupt = await asyncio.wait_for(is_interrupt_task, timeout=.5)
                if is_interrupt:
                    return True
            except Exception as e:
                self.logger.debug(f"Timeout on interrupt model, {repr(e)}")
        ...
        if not self.transcriber_config.endpointing_config:
            ...
            if is_finished and self.transcriber_config.skip_on_back_track_audio:
                ...
                try:
                    is_skip = await asyncio.wait_for(is_interrupt_task, timeout=.5)
                    if is_skip:
                        return False
                except Exception as e:
                    self.logger.debug(f"Timeout on skip model {repr(e)}")
            return is_finished
